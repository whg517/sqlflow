...
import { mockApiRoutes, loginViaUI, setToken, MOCK_TICKET, MOCK_CREATED_TICKET } from '../../support/mock-routes'
...
test.describe('工单操作', () => {
  ...
  test('工单列表显示数据', async ({ page }) => {
    ...
    // 使用 badge 定位器避免 strict mode 冲突
    await expect(page.getByRole('table').getByText('待审批')).toBeVisible()
    await expect(page.getByRole('table').getByText('高风险')).toBeVisible()
  })
  ...
  test('工单详情抽屉打开', async ({ page }) => {
    ...
    await expect(page.locator('[data-slot="sheet-content"]').getByText('高风险')).toBeVisible()
  })
  ...
  test('工单详情 SQL 内容可复制', async ({ page }) => {
    ...
    // 验证 SQL 内容显示
    await expect(page.locator('[data-slot="sheet-content"]').getByText(MOCK_TICKET.sql_content)).toBeVisible()
  ...
  test('工单详情显示提交人信息', async ({ page }) => {
    ...
    // 验证提交人信息
    await expect(page.locator('[data-slot="sheet-content"]').getByText('提交人: admin')).toBeVisible()
  })
