...
import { listSensitiveTables, type SensitiveTable } from "@/features/security/api/maskRule";
...
export function useSensitiveTables(datasourceId: number | null) {
  const [fetchedMap, setFetchedMap] =
    useState<Map<string, SensitiveTable>>(EMPTY_MAP);
  ...
  const isSensitive = useCallback(
    (tableName: string): SensitiveTable | undefined => {
