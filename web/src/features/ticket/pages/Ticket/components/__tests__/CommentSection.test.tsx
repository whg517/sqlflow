...
vi.mock("@/features/ticket/api/comment", () => ({
...
import CommentSection from "@/features/ticket/pages/Ticket/components/CommentSection";
...
function renderCommentSection(props: Record<string, unknown> = {}) {
  ...
  return render(
    <CommentSection {...defaults} {...(props as Record<string, unknown>)} />,
  );
}
...
describe("CommentSection", () => {
  ...
  describe("submit comment", () => {
    it("enables submit button when content is typed", async () => {
      ...
      const sendBtn = buttons.find(
        (b) => b.querySelector("svg.lucide-send") || b.textContent === "",
      );
  ...
  describe("delete comment", () => {
    it("shows delete button for own comments", async () => {
      ...
      const { rerender } = renderCommentSection({
        orderId: 0,
        currentUserId: 10,
