...
vi.mock("@/shared/components/ui/select", () => ({
  Select: ({
    ...
    (globalThis as Record<string, unknown>).__selectOnValueChange =
      onValueChange;
    ...
    <div
      ...
      onClick={() => {
        const cb = (globalThis as Record<string, unknown>)
          .__selectOnValueChange as ((v: string) => void) | undefined;
        cb?.(value);
      }}
    >
      {children}
...
import TicketNewPage from "@/features/ticket/pages/TicketNew";
...
describe("TicketNewPage", () => {
  ...
  describe("rendering", () => {
    ...
    it('renders "数据库名" input', () => {
      renderTicketNewPage();
      expect(
        screen.getByPlaceholderText("输入数据库名（可选）"),
      ).toBeInTheDocument();
    });
