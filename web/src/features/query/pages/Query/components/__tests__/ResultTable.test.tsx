...
// Document sources put objects and arrays in cells. Rendering them with
// String() produced "[object Object]", losing the value entirely.
describe("nested cell values", () => {
  it("renders a nested object as JSON rather than [object Object]", () => {
    render(
      <ResultTable
        result={{
          shape: "documents",
          columns: ["_id", "address"],
          rows: [{ _id: "1", address: { city: "SF", zip: "94107" } }],
          total: 1,
          execution_time_ms: 1,
          affected_rows: 0,
          desensitized: false,
    ...
    expect(
      screen.getByText('{"city":"SF","zip":"94107"}'),
    ).toBeInTheDocument();
  });

  it("renders an array cell as JSON", () => {
    render(
      <ResultTable
        result={{
          shape: "documents",
