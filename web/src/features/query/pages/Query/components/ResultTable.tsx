...
import { createTable, getCoreRowModel } from "@/shared/lib/tanstack-table";
import {
  Table,
  TableHeader,
  TableBody,
...
function applyFilterOperator(
  ...
  const cellStr = formatCellValue(rowValue);
...
// formatCellValue renders a cell as text.
//
// Document sources (Elasticsearch hits, Mongo documents) put objects and arrays
// in cells. String() turns those into "[object Object]", which loses the value
// entirely, so structured values are shown as compact JSON instead.
  ...
  const columns = useMemo<ColumnDef<Record<string, unknown>>[]>(() => {
    ...
    return result.columns.map((col) => ({
      ...
      filterFn: ((row: any, _id: string, value: any) => {
        const colVal = formatCellValue(row.getValue(col)).toLowerCase();
