...
interface QueryStore {
  ...
  updateTabDatasource: (id: string, datasourceId: number | null, database: string, datasourceType?: string) => void
  setTabResult: (id: string, result: QueryResult | null, error?: string | null) => void
  setTabExecuting: (id: string, executing: boolean) => void
  restoreHistoryAsTab: (sql: string, datasourceId: number, database: string) => void
  // MongoDB actions
  updateMongoField: (id: string, field: Partial<Pick<QueryTab, 'mongoCollection' | 'mongoOperation' | 'mongoFilter' | 'mongoOptions'>>) => void
  // AI review actions
...
export const useQueryStore = create<QueryStore>((set) => ({
  ...
  updateTabDatasource: (id, datasourceId, database, datasourceType) =>
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === id ? { ...t, datasourceId, database, datasourceType: datasourceType ?? t.datasourceType } : t,
      ),
    })),
  ...
  setTabResult: (id, result, error = null) =>
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === id ? { ...t, result, error, executing: false, dirty: false } : t,
      ),
    })),
  ...
  restoreHistoryAsTab: (sql, datasourceId, database) => {
    ...
    tab.title = sql.trim().substring(0, 20) + (sql.trim().length > 20 ? '...' : '')
    tab.dirty = false
    set((state) => ({
      tabs: [...state.tabs, tab],
      activeTabId: tab.id,
  ...
  setAIReviewResult: (id, result) =>
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === id ? { ...t, aiReviewResult: result, aiReviewStatus: result ? 'done' : 'idle' } : t,
      ),
    })),
  ...
  setAIReviewError: (id, error) =>
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === id ? { ...t, aiReviewError: error, aiReviewStatus: error ? 'error' : 'idle' } : t,
      ),
    })),
  ...
  clearAIReview: (id) =>
    set((state) => ({
      tabs: state.tabs.map((t) =>
        t.id === id ? { ...t, aiReviewStatus: 'idle', aiReviewResult: null, aiReviewContent: '', aiReviewError: null } : t,
      ),
    })),
