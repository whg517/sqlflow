...
describe('queryStore', () => {
  ...
  describe('updateTabSql', () => {
    ...
    it('sets dirty flag to true when SQL is updated', async () => {
      ...
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.dirty).toBe(true)
    })
  ...
  describe('setTabExecuting', () => {
    it('sets executing flag for a tab', async () => {
      ...
      store.setTabExecuting(tabId, true)
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.executing).toBe(true)

      store.setTabExecuting(tabId, false)
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.executing).toBe(false)
    })
    ...
    it('clears error when starting execution', async () => {
      ...
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.error).toBeNull()
    })
  })
  ...
  describe('AI review actions', () => {
    it('setAIReviewStatus updates review status', async () => {
      ...
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.aiReviewStatus).toBe('reviewing')
    })
    ...
    it('appendAIReviewContent appends text', async () => {
      ...
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.aiReviewContent).toBe('Hello World')
    })
    ...
    it('setAIReviewError resets status when error is null', async () => {
      ...
      expect(useQueryStore.getState().tabs.find((t) => t.id === tabId)?.aiReviewStatus).toBe('idle')
    })
  ...
  describe('restoreHistoryAsTab', () => {
    ...
    it('sets title from SQL content', async () => {
      const { useQueryStore } = await import('@/store/queryStore')
      useQueryStore.getState().restoreHistoryAsTab('SELECT * FROM users', 10, 'db')
    ...
    it('truncates long SQL title', async () => {
      const { useQueryStore } = await import('@/store/queryStore')
      const longSql = 'SELECT * FROM very_long_table_name WHERE some_column = some_value AND another_column > 0'
