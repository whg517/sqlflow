...
export async function listMaskRules(params?: {
  ...
  return api.get<MaskRuleListResponse>(
    `/mask-rules${query ? `?${query}` : ""}`,
  );
}
...
export async function listSensitiveTables(params?: {
  ...
  if (params?.table_name) qs.set("table_name", params.table_name);
  const query = qs.toString();
  return api.get<SensitiveTableListResponse>(
    `/sensitive-tables${query ? `?${query}` : ""}`,
  );
}

export async function createSensitiveTable(
  req: CreateSensitiveTableRequest,
): Promise<ApiResponse<SensitiveTable>> {
  return api.post<ApiResponse<SensitiveTable>>("/sensitive-tables", req);
}

export async function deleteSensitiveTable(
  id: number,
): Promise<ApiResponse<{ message: string }>> {
...
export async function fetchDatasourceTables(
  datasourceId: number,
