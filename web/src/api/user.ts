...
export async function listUsers(page = 1, pageSize = 20): Promise<ListUsersResponse> {
  return api.get<ListUsersResponse>(`/users?page=${page}&page_size=${pageSize}`)
}

export async function createUser(req: CreateUserRequest): Promise<UserActionResponse> {
  return api.post<UserActionResponse>('/users', req)
}

export async function updateUser(id: number, req: UpdateUserRequest): Promise<UserActionResponse> {
...
export async function resetPassword(id: number, req: ResetPasswordRequest): Promise<UserActionResponse> {
