...
describe('MaskRule API Helpers', () => {
  ...
  describe('SENSITIVITY_BADGE', () => {
    it('has correct entries for low, medium, high', () => {
      expect(SENSITIVITY_BADGE.low).toEqual({ label: '低', cls: 'bg-emerald-500/20 text-emerald-400' })
      expect(SENSITIVITY_BADGE.medium).toEqual({ label: '中', cls: 'bg-yellow-500/20 text-yellow-400' })
      expect(SENSITIVITY_BADGE.high).toEqual({ label: '高', cls: 'bg-red-500/20 text-red-400' })
    })
  })
