...
describe('Ticket API Helpers', () => {
  ...
  describe('getStatusColor', () => {
    ...
    it('each status has a unique color class', () => {
      const statuses: TicketStatus[] = [
        'SUBMITTED', 'AI_REVIEWED', 'PENDING_APPROVAL',
        'APPROVED', 'REJECTED', 'EXECUTING', 'DONE', 'CANCELLED',
      ]
    ...
    it('returns default color for unknown status', () => {
      expect(getStatusColor('UNKNOWN' as TicketStatus)).toBe('bg-gray-500/20 text-gray-400')
    })
  })
  ...
  describe('formatTime', () => {
    it('formats ISO date string to MM-DD HH:mm', () => {
      expect(formatTime('2026-05-23T10:30:00Z')).toMatch(/^\d{2}-\d{2} \d{2}:\d{2}$/)
    })
