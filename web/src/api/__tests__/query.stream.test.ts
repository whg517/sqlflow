...
describe('Query API - streamAIReview', () => {
  ...
  it('calls onEvent for each SSE event', async () => {
    ...
    expect(onEvent).toHaveBeenCalledWith({ type: 'content', data: 'Analyzing...' })
    expect(onEvent).toHaveBeenCalledWith({ type: 'result', data: { risk_level: 'low' } })
  })
  ...
  it('sends correct request headers', async () => {
    ...
    streamAIReview({ datasource_id: 1, database: 'db', sql: 'SELECT 1' }, onEvent, onError)
  ...
  it('handles non-ok response with error message', async () => {
    ...
    expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'SQL too complex' }))
  })
  ...
  it('cancel function aborts the request', () => {
    ...
    const cancel = streamAIReview({ datasource_id: 1, database: 'db', sql: 'SELECT 1' }, onEvent, onError)
...
describe('Query API - exportQuery', () => {
  ...
  it('downloads CSV file', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/query/export', expect.objectContaining({
      method: 'POST',
    }))
  ...
  it('handles 401 response', async () => {
    ...
    await expect(
      exportQuery({ datasource_id: 1, database: 'db', sql: 'SELECT 1', format: 'csv' })
