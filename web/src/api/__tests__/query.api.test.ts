...
describe('Query API Functions', () => {
  ...
  describe('executeQuery', () => {
    it('executes a query and returns result', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/query/execute', expect.objectContaining({ method: 'POST' }))
    })

    it('sends correct request body', async () => {
      mockJson({ code: 0, data: { columns: [], rows: [], total: 0, execution_time_ms: 0, affected_rows: 0, desensitized: false, desensitized_fields: [], warnings: [] } })
      await executeQuery({ datasource_id: 5, database: 'production', sql: 'SELECT 1' })
      const body = JSON.parse(mockFetch.mock.calls[0][1].body)
      expect(body).toEqual({ datasource_id: 5, database: 'production', sql: 'SELECT 1' })
    })
  })
  ...
  describe('deleteHistory', () => {
    it('deletes a history entry', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/query/history/1', expect.objectContaining({ method: 'DELETE' }))
    })
  })
  ...
  describe('clearHistory', () => {
    it('clears all history', async () => {
      ...
      expect(res.code).toBe(0)
      expect(mockFetch).toHaveBeenCalledWith('/api/query/history', expect.objectContaining({ method: 'DELETE' }))
    })
  })
})
