...
export function useSchemaCompletion() {
  ...
  const fetchTables = useCallback(async (datasourceId: number): Promise<SchemaData | null> => {
  ...
  const fetchColumns = useCallback(async (datasourceId: number, tableName: string): Promise<string[]> => {
