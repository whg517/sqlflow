...
describe("HighlightText", () => {
  ...
  it("highlights all occurrences of the keyword", () => {
    render(
      <HighlightText
        text="SELECT id, SELECT name FROM SELECT"
        keyword="SELECT"
      />,
    );
  ...
  it("safely handles HTML injection in keyword (no XSS)", () => {
    render(
      <HighlightText text="<script>alert('xss')</script>" keyword="<script>" />,
    );
  ...
  it("safely handles HTML injection in text (no XSS)", () => {
    render(
      <HighlightText text="<img onerror=alert(1)>hello" keyword="hello" />,
    );
