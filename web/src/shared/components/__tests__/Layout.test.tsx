...
vi.mock("@/shared/components/ChangePasswordDialog", () => ({
  default: ({ open }: { open: boolean }) =>
    open ? (
      <div data-testid="change-password-dialog">ChangePasswordDialog</div>
    ) : null,
}));
...
vi.mock("@/shared/components/NetworkBanner", () => ({
      ...
      <TooltipProvider>
        <Routes>
          <Route element={<Layout />}>
            ...
            <Route
              path="query"
              element={<div data-testid="outlet-page">Query</div>}
            />
            <Route
              path="tickets"
              element={<div data-testid="outlet-page">Tickets</div>}
            />
          </Route>
        </Routes>
      </TooltipProvider>
  ...
  describe("sidebar navigation", () => {
    ...
    it("does not render user management nav item for non-admin user", async () => {
      mockApiGet.mockResolvedValue({
        code: 0,
        data: { username: "devuser", role: "developer" },
      });
      renderLayout();
      await waitFor(() => {
