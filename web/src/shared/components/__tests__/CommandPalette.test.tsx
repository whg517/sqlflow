...
vi.mock("@/features/query/api/query", () => ({
...
vi.mock("@/features/audit/api/audit", () => ({
...
describe("CommandPalette", () => {
  ...
  describe("search results", () => {
    it("renders query history results with highlighted text", async () => {
      ...
      expect(
        screen.getByText("orders", { selector: "mark" }),
      ).toBeInTheDocument();
  ...
  describe("API failure handling (Promise.allSettled)", () => {
    it("shows partial error when 1 API fails", async () => {
      mockSearchQueryHistory.mockImplementation((kw: string) =>
        kw
          ? Promise.reject(new Error("Query API down"))
          : Promise.resolve({ data: [], page: 1, page_size: 5, total: 0 }),
      );
      mockListTickets.mockResolvedValue({
        data: [ticketItem],
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/查询历史: Query API down/),
        ).toBeInTheDocument();
      });
