...
export function SensitiveTableBadge({
  ...
  return (
    <Badge
      className={`gap-0.5 border-0 ${badge.cls} ${sizeClasses}`}
    >
 ...
 */
export function SensitiveTableName({ tableName, sensitivityLevel }: SensitiveTableNameProps) {
  const levelBg = sensitivityLevel === 'high'
    ? 'bg-red-500/20'
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      <span className={`rounded ${levelBg} px-1.5 py-0.5 font-medium text-[var(--text-primary)]`}>
        {tableName}
      </span>
    </span>
  )
