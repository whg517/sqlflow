...
function validatePassword(v: string): string | null {
  ...
  if (!/[a-zA-Z]/.test(v) || !/[0-9]/.test(v)) return '密码必须包含至少一个字母和一个数字'
  return null
}
...
export default function UsersPage() {
  ...
  const [createForm, setCreateForm] = useState({ username: '', password: '', role: 'developer' })
  ...
  async function handleEditSubmit(e: FormEvent) {
    ...
    setEditLoading(true)
    try {
      await updateUser(editUser.id, { username: editForm.username, role: editForm.role })
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <Badge className={ROLE_BADGE_CLASS[role] ?? 'bg-gray-500/20 text-gray-400'}>
          ...
          return (
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                title={isAdmin ? '不能禁用管理员' : user.status === 'disabled' ? '已禁用' : '禁用用户'}
              >
                <Ban size={13} />
                禁用
              </Button>
    ...
    <div className="flex h-full flex-col">
              ...
              <TableBody>
                {loading && !users.length ? (
                  ...
                  table.getRowModel().rows.map((row) => (
                    <TableRow key={row.id} className="border-[var(--border-default)]">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="border-[var(--border-default)] bg-[var(--bg-surface)] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[var(--text-primary)]">新建用户</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="border-[var(--border-default)] bg-[var(--bg-surface)] sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[var(--text-primary)]">编辑用户 — {editUser?.username}</DialogTitle>
          </DialogHeader>
      ...
      >
        <AlertDialogContent className="border-[var(--border-default)] bg-[var(--bg-surface)]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-[var(--text-secondary)]">
              确定要禁用用户「{disableTarget?.username}」吗？禁用后该用户将无法登录。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-[var(--border-default)]">
