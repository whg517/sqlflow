...
vi.mock('@/api/audit', () => ({
  ...
  formatExecutionTime: (ms: number) => ms < 1000 ? `${ms}ms` : `${(ms / 1000).toFixed(2)}s`,
...
vi.mock('@/components/ui/select', () => ({
  Select: ({ children, value, onValueChange }: { children: React.ReactNode; value: string; onValueChange: (v: string) => void }) => {
    ;(globalThis as Record<string, unknown>).__auditSelectOnValueChange = onValueChange
  ...
  SelectItem: ({ children, value }: { children: React.ReactNode; value: string }) =>
    <div data-testid="select-item" data-value={value} onClick={() => {
      const cb = (globalThis as Record<string, unknown>).__auditSelectOnValueChange as ((v: string) => void) | undefined
      cb?.(value)
    }}>{children}</div>,
}))
...
const mockLogs = [
  {
    id: 1, user_id: 1, username: 'admin', action: 'SELECT', datasource_id: 10,
    database: 'testdb', sql_content: 'SELECT * FROM users WHERE id = 1',
    sql_summary: 'SELECT * FROM users...', result_rows: 1, affected_rows: 0,
    execution_time_ms: 45, error_message: '', desensitized_fields: '', ip_address: '192.168.1.1',
    created_at: '2026-05-24T10:00:00Z',
  },
  {
    id: 2, user_id: 2, username: 'developer', action: 'UPDATE', datasource_id: 11,
    database: 'proddb', sql_content: 'UPDATE orders SET status = "done" WHERE id = 5',
    sql_summary: 'UPDATE orders...', result_rows: 0, affected_rows: 1,
    execution_time_ms: 120, error_message: '', desensitized_fields: 'email,phone',
    ip_address: '10.0.0.5', created_at: '2026-05-24T11:00:00Z',
  },
]
...
function setupMocks() {
  ...
  mockListAuditLogs.mockResolvedValue({
    data: mockLogs, page: 1, page_size: 50, total: 2,
  })
...
describe('AuditPage', () => {
  ...
  describe('empty state', () => {
    it('shows "暂无审计日志" when no logs', async () => {
      mockListAuditLogs.mockResolvedValue({ data: [], page: 1, page_size: 50, total: 0 })
      renderAuditPage()
      await waitFor(() => {
  ...
  describe('pagination', () => {
    it('shows pagination when multiple pages', async () => {
      ...
      mockListAuditLogs.mockResolvedValue({ data: manyLogs.slice(0, 50), page: 1, page_size: 50, total: 55 })
