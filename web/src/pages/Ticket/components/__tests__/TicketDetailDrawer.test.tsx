...
vi.mock('@/api/ticket', () => ({
  ...
  getStatusLabel: (status: string) => {
    const map: Record<string, string> = {
      SUBMITTED: '已提交', AI_REVIEWED: 'AI 已评审', PENDING_APPROVAL: '待审批',
      APPROVED: '已通过', REJECTED: '已拒绝', EXECUTING: '执行中',
...
vi.mock('@/components/ui/alert-dialog', () => ({
  ...
  AlertDialogAction: ({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: React.ReactNode }) =>
    <button onClick={onClick} disabled={disabled} data-testid="alert-action">{children}</button>,
  AlertDialogCancel: ({ disabled, children }: { disabled?: boolean; children: React.ReactNode }) =>
    <button disabled={disabled} data-testid="alert-cancel">{children}</button>,
}))

// Mock Button
vi.mock('@/components/ui/button', () => ({
  Button: ({ onClick, disabled, children, className }: { onClick?: () => void; disabled?: boolean; children: React.ReactNode; className?: string }) =>
...
function renderDrawer(overrides: Partial<Parameters<typeof TicketDetailDrawer>[0]> = {}) {
  const props = {
    open: true,
    onOpenChange: vi.fn(),
    ticketId: 1,
...
describe('TicketDetailDrawer', () => {
  ...
  describe('rendering', () => {
    ...
    it('displays change reason', async () => {
      renderDrawer()
      await waitFor(() => {
        expect(screen.getByText('Adding age column for profile feature')).toBeInTheDocument()
      })
    })
  ...
  describe('AI review display', () => {
    it('shows AI review section when ai_review_result is present', async () => {
      mockGetTicket.mockResolvedValue({
        data: {
          ...
          ai_review_result: JSON.stringify({
            ...
            suggestions: ['Consider adding a DEFAULT value', 'Add NOT NULL constraint'],
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('This ALTER TABLE adds a new column')).toBeInTheDocument()
        expect(screen.getByText('Consider adding a DEFAULT value')).toBeInTheDocument()
        expect(screen.getByText(/Low impact, additive change/)).toBeInTheDocument()
      })
    })
  ...
  describe('approve action', () => {
    ...
    it('calls approveTicket on confirm with comment', async () => {
      mockApproveTicket.mockResolvedValue({ data: { ...baseTicket, status: 'APPROVED' } })
      ...
      await userEvent.click(actions.find(a => a.textContent?.includes('确认通过'))!)

      await waitFor(() => {
        expect(mockApproveTicket).toHaveBeenCalledWith(1, 'Approved, looks good')
      })
    })
  })
  ...
  describe('reject action', () => {
    ...
    it('shows error toast when rejecting without reason', async () => {
      ...
      mockGetTicket.mockResolvedValue({ data: baseTicket })
      mockRejectTicket.mockResolvedValue({ data: { ...baseTicket, status: 'REJECTED' } })
      ...
      await waitFor(() => {
        const actions = screen.getAllByTestId('alert-action')
        const rejectAction = actions.find(a => a.textContent?.includes('确认驳回'))
    ...
    it('calls rejectTicket with reason on confirm', async () => {
      ...
      const actions = screen.getAllByTestId('alert-action')
      await userEvent.click(actions.find(a => a.textContent?.includes('确认驳回'))!)

      await waitFor(() => {
        expect(mockRejectTicket).toHaveBeenCalledWith(1, 'Too risky for production')
      })
    })
  })
  ...
  describe('cancel action', () => {
    ...
    it('calls cancelTicket with reason on confirm', async () => {
      mockCancelTicket.mockResolvedValue({ data: { ...baseTicket, status: 'CANCELLED' } })
      mockGetTicket
        .mockResolvedValueOnce({ data: { ...baseTicket, status: 'SUBMITTED' } })
        .mockResolvedValueOnce({ data: { ...baseTicket, status: 'CANCELLED' } })
      ...
      const actions = screen.getAllByTestId('alert-action')
      await userEvent.click(actions.find(a => a.textContent?.includes('确认取消'))!)
  ...
  describe('execute action', () => {
    ...
    it('calls executeTicket on confirm', async () => {
      mockExecuteTicket.mockResolvedValue({ data: { ...baseTicket, status: 'EXECUTING' } })
      mockGetTicket
        .mockResolvedValueOnce({ data: { ...baseTicket, status: 'APPROVED' } })
        .mockResolvedValueOnce({ data: { ...baseTicket, status: 'EXECUTING' } })
      ...
      const actions = screen.getAllByTestId('alert-action')
      await userEvent.click(actions.find(a => a.textContent?.includes('确认执行'))!)
  ...
  describe('action feedback', () => {
    ...
    it('shows error toast on action failure', async () => {
      ...
      mockGetTicket.mockResolvedValue({ data: { ...baseTicket, status: 'APPROVED' } })
  ...
  describe('SQL copy', () => {
    it('has copy button for SQL content', async () => {
      ...
      const copyBtn = document.querySelector('button[onclick]') || screen.getByText('ALTER TABLE users ADD COLUMN age INT').closest('div')?.querySelector('button')
