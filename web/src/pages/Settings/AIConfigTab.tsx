...
const MODEL_OPTIONS: Record<string, { value: string; label: string }[]> = {
  ...
  custom: [{ value: "custom", label: "输入模型名称" }],
};
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      if (ai) {
        ...
        const opts =
          MODEL_OPTIONS[ai.provider || "openai"] || MODEL_OPTIONS.custom;
        ...
        setBaseUrl(
          ai.base_url ||
            DEFAULT_BASE_URLS[ai.provider || "openai"] ||
      ...
      <div className="flex items-center gap-3 rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] p-4">
        {config?.enabled ? (
          <>
            ...
            <Badge className="ml-1 border-0 bg-emerald-500/20 text-emerald-400 text-[10px]">
              已启用
            </Badge>
          </>
        ) : (
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder={
            config?.api_key
              ? "留空保持当前 Key 不变"
              : "输入 API Key 以启用 AI 评审"
          }
          className="border-[var(--border-default)] bg-[var(--bg-elevated)]"
        />
        {!config?.enabled && !apiKey && (
          <p className="text-xs text-yellow-400">
            未配置 API Key，AI 评审将使用静态规则
          </p>
        )}
      </div>
    ...
    <form onSubmit={handleSave} className="space-y-6">
      ...
      <div className="flex items-center gap-3 rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] p-4">
        {config?.enabled ? (
          <>
            ...
            <Badge className="ml-1 border-0 bg-blue-500/20 text-blue-400 text-[10px]">
              已启用
            </Badge>
          </>
        ) : (
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder={
            config?.secret ? "留空保持当前密钥不变" : "可选，机器人加签密钥"
          }
          className="border-[var(--border-default)] bg-[var(--bg-elevated)]"
        />
      </div>
