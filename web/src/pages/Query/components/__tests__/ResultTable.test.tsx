...
describe('ResultTable column filters', () => {
  ...
  it('renders filter icon buttons for each column', () => {
    ...
    const filterButtons = document.querySelectorAll('[data-slot="popover-trigger"]')
  ...
  it('opens filter popover when clicking the filter icon', async () => {
    ...
    const filterTrigger = nameHeader.querySelector('[data-slot="popover-trigger"]')!
    ...
    expect(within(popover!).getByPlaceholderText('输入筛选值...')).toBeInTheDocument()
  })
  ...
  it('column sorting works and sort indicator is visible', async () => {
    ...
    const sortArea = nameThFound!.querySelector('.cursor-pointer') as HTMLElement
    expect(sortArea).toBeTruthy()
    await user.click(sortArea)

    // Wait for state update to propagate
    await vi.waitFor(() => {
      expect(nameThFound!.querySelector('svg.lucide-chevron-up')).toBeInTheDocument()
    })
  })
  ...
  it('resets filters when result changes', async () => {
    ...
    const result2 = makeResult({ rows: [{ id: 10, name: 'Dave', status: 'active' }] })
