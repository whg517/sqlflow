...
import { fetchHistory, deleteHistory, clearHistory, type QueryHistoryItem } from '@/api/query'
...
export default function HistoryPanel() {
  ...
  return (
    <>
      <div data-history-panel className="absolute right-0 top-full z-20 mt-1 w-[380px] rounded-lg border border-[var(--border-default)] bg-[var(--bg-surface)] shadow-lg">
        <div className="flex items-center justify-between border-b border-[var(--border-default)] px-3 py-2">
          <span className="text-xs font-medium text-[var(--text-primary)]">查询历史</span>
            ...
            <div className="py-1">
              {items.map((item) => (
                <div
                  ...
                  onClick={() => restoreHistoryAsTab(item.sql_content, item.datasource_id, item.database)}
                >
                  <Clock size={12} className="mt-1 shrink-0 text-[var(--text-muted)]" />
      ...
      <AlertDialog open={confirmClear} onOpenChange={setConfirmClear}>
        <AlertDialogContent className="border-[var(--border-default)] bg-[var(--bg-surface)]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[var(--text-primary)]">确认清空</AlertDialogTitle>
