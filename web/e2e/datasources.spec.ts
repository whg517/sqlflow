...
import { mockApiRoutes, setToken, MOCK_DATASOURCES } from './helpers'
...
test.describe('数据源管理', () => {
  ...
  test('导航到数据源设置页', async ({ page }) => {
    ...
    // 验证左侧导航高亮
    await expect(page.getByRole('button', { name: '数据源' })).toHaveClass(/accent-primary/)
  })
  ...
  test('添加数据源对话框可以打开', async ({ page }) => {
    ...
    await expect(page.getByText('添加数据源')).toBeVisible()
  ...
  test('设置页子导航切换', async ({ page }) => {
    ...
    // 切换到脱敏规则
    await page.getByRole('button', { name: '脱敏规则' }).click()
    await expect(page.getByRole('button', { name: '脱敏规则' })).toHaveClass(/accent-primary/)
    await expect(page.getByRole('button', { name: '数据源' })).not.toHaveClass(/accent-primary/)

    // 切换到 AI 配置
    await page.getByRole('button', { name: 'AI 配置' }).click()
    await expect(page.getByRole('button', { name: 'AI 配置' })).toHaveClass(/accent-primary/)
  })
  ...
  test('数据源类型选择 MySQL 和 MongoDB', async ({ page }) => {
    ...
    // 默认选择 MySQL
    // 在 SelectTrigger 中，验证默认类型为 mysql
    await expect(page.getByText('MySQL')).toBeVisible()

    // 点击选择器打开下拉
    const typeSelect = page.locator('form').getByText('MySQL').first()
