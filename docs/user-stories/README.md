...
> User Story 描述目标行为和验收标准，不代表当前实现已经通过验收。实际状态以[需求文档](../REQUIREMENTS.md)为准；当前阻断项和修复顺序见[跨角色评审](../reviews/2026-07-26-cross-functional-review.md)与[整改路线图](../ROADMAP.md)。
...
5. 至少有一层自动化测试（Go 测试或 Vitest）。
6. 需求、架构或 ADR 因该故事发生变化时已同步更新。

当前代码中已有接口、页面或单元测试，不足以单独把故事判定为完成。评审发现的越权路径、角色主流程断点、调度/恢复缺陷必须作为失败验收场景补入自动化测试。
