...
> 事实源：以代码为唯一事实源（`internal/service/`、`internal/model/model.go`、`web/src/`、`e2e/tests/`）；文档（PRD/api.md）与代码不一致处以代码为准并显式标注
> 关联文档：[PRD-v2.md](./spec/PRD-v2.md)、[UI-DESIGN-v2.md](./spec/UI-DESIGN-v2.md)、[api.md](./api.md)、[ARCHITECTURE-v2.md](./spec/ARCHITECTURE-v2.md)
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.0 | 2026-07-04 | 初版：按功能模块梳理 9 大类 42 条旅程、4 个状态机、24 条异常路径 |
| v2.0 | 2026-07-04 | **重构**：补充 4 个新维度（微观交互 / 多数据源差异 / 实体生命周期 / 真实场景），新增 §11 场景旅程（50 条真实场景）、§12 多数据源差异矩阵、§13 微观交互状态机、§14 实体生命周期；旅程总数 42 → **90+**；修正 v1.0 中"ES 无权限检查"等事实错误；新增旅程地图要素（情绪/痛点/机会点） |
...
> 工单状态机共 10 个状态。⚠️ **文档不一致**：PRD-v2 称"6 态"，api.md 列 8 个状态名，**代码实际 10 态**（多 `SCHEDULED`、`FAILED`）。以代码为准。
...
| # | 文档 | 代码 | 影响 |
|---|------|------|------|
| 1 | 工单状态机 6/8 态 | **实际 10 态**（多 SCHEDULED、FAILED） | 状态分析须以代码为准 |
| 2 | CSS token indigo `#6366f1` | **橙色 `#ea580c`** | 视觉规范以 index.css 为准 |
| 3 | PRD-v2 排除"多级审批/钉钉 OAuth/FTS5/性能监控" | **代码全部已实现** | PRD-v2 已严重落后，需求以 README+api.md 为准 |
| 4 | CHANGELOG 停在 v1.0.0 | PG/ES/审批引擎/SLA/OIDC/飞书等已演进 | 演进未记录 |
| 5 | README 声称支持钉钉 OAuth2 登录 | 钉钉仅作通知渠道，无独立登录端点 | README 表述需修正 |
...
| 维度 | v1.0 | v2.0 | 增量 |
|------|------|------|------|
| 分析维度 | 6 个 | **10 个** | +微观交互、多数据源差异、实体生命周期、真实场景 |
| 旅程数量 | 42 条功能旅程 | 42 + **50 条场景旅程** = 90+ | 场景维度全覆盖 |
| 状态机 | 4 个 | 4 + **微观 UI 状态机** | 含 5 种 AI decision、按钮矩阵、结果表交互 |
| 差异分析 | 无 | **四数据源 × 7 维度差异矩阵** | 含"能做/不能做"清单 |
| 实体生命周期 | 无 | **15 个实体完整生命周期** | 含子实体关系 |
| 旅程地图要素 | 无 | **情绪/痛点/机会点**（场景 S05/S10） | 真正的 User Journey Map |
| 交叉验证 | 6 维 | 10 维 + **逐条代码核实** | 修正 v1.0 事实错误 |

### 15.2 修正的 v1.0 事实错误

| v1.0 错误描述 | v2.0 修正（交叉验证后） |
|---------------|------------------------|
| "ES 完全无权限检查" | ❌ 错误。query.go:149-158 的 Casbin 检查对所有数据源统一生效（在 dbType 分支之前） |
| "通知偏好过滤存在但未接线" | ✅ 确认正确（notify.go 无 ShouldNotify 调用） |
| "通用 Webhook 未接入工单生命周期" | ✅ 确认正确（DeliverEvent 无业务调用点，SLA 事件收不到） |

### 15.3 新发现的缺口（v2.0 新增）

| # | 缺口 | 影响 |
|---|------|------|
| 9 | **多个清理任务未注册定时** | 权限过期/Refresh Token/Share 过期清理依赖手动调用，temp_policies 可能残留 |
| 10 | **工单执行不校验数据源状态** | disabled 数据源的已批工单仍可执行 |
| 11 | **User 无 status 字段** | "禁用"只能删除，与前端"禁用"按钮语义不符 |
| 12 | **JWT 不可主动撤销** | 改密/重置后 access token 在过期前仍有效 |
| 13 | **GitLink 无级联删除** | 删工单不删关联 GitLink，残留数据 |
| 14 | **列筛选无唯一值下拉** | 设计预期 ≤200 唯一值列表未实现，只有手输值 |
| 15 | **MySQL 工单 DDL 不可回滚** | 多条 DDL 中途失败产生中间态，真实生产风险 |
| 16 | **PG 用 MySQL 方言编辑器** | RETURNING/$1/::cast 高亮不准 |
| 17 | **Mongo/ES 池参数不生效** | UI 配置的 max_open 等对 NoSQL 无效但不提示 |
| 18 | **SLA 通知不走飞书/Webhook** | 仅走钉钉，与"多通道通知"宣传不符 |

### 15.4 建议的后续动作

| 优先级 | 动作 | 对应缺口 |
|--------|------|----------|
| P0 | 实现 `auto_skip_same_submitter` 或移除该字段 | §9.1 #1 |
| P0 | 注册过期清理定时任务（权限/Token/Share/RefreshToken） | §9.1 #2、§15.3 #9 |
| P1 | 工单执行前校验数据源状态 | §15.3 #10 |
| P1 | 接线通知偏好过滤到 Notify* 方法 | §9.1 #6 |
| P1 | 接线通用 Webhook DeliverEvent 到工单/SLA 生命周期 | §9.1 #3 |
| P2 | 统一确认对话框（移除原生 confirm） | §13.4 |
| P2 | 列筛选实现唯一值下拉 | §15.3 #14 |
| P2 | PG 编辑器方言支持 | §15.3 #16 |
| P3 | OIDC state 用 session/Redis 校验 | §9.1 #5 |
| P3 | User 表增加 status 字段支持软禁用 | §15.3 #11 |
