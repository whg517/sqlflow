...
- 审计检索、报表、通知/Webhook、平台 PostgreSQL 备份（`pg_dump`）、健康检查和指标。

### 3.2 术语

| 术语 | 定义 |
|---|---|
| 平台元数据 | SQLFlow 自身的用户、策略、工单、审计等数据，存储于 PostgreSQL（ADR-0009） |
...
- 平台元数据库为 PostgreSQL（ADR-0009），写并发不再受单连接限制；但调度任务仍在进程内运行且无分布式租约，故目前仍只支持单副本部署。
