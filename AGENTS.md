...
- **状态文件**: `../../state/task-progress.json`（workspace 根目录）
- **开发计划**: `docs/proposals/001-mvp-initial/plan.md`
- **Spec 文档**: `docs/spec/` 目录

## 任务派发与执行

### 小梦旋派发任务时

1. 从 `plan.md` 中提取下一个待执行的 Task
...
1. `go build ./...` 编译检查
2. `go test ./...` 测试检查
3. `git diff --stat` 查看变更范围
4. 检查是否破坏现有代码
5. 验收通过 → 更新状态 + 提交 + 汇报化刚
6. 验收失败 → 记录问题 + 重新派发或手动修复
...
- [ ] 没有破坏现有 handler/service 的 import
- [ ] 符合 Spec 文档的功能要求
- [ ] 新代码有对应的测试文件
...
- 每个 Phase 完成后提交一个 commit，格式：`feat(phase-N): 简要描述`
- 不要在 commit 中混入不相关修改
