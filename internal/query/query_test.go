...
// and seed casbin rules pre-populated (so security.NewService won't try to
// read policy.csv from disk).
// testActorID is the user the masking tests act as.
//
// These tests used to go through an applyDesensitization shim that hardcoded
// userID 0, so the per-user unmask grant was never on the path they exercised —
// only the role-level one. Naming the actor makes which of the two is under test
// visible at the call site.
const testActorID = int64(1)
...
// so that security.NewService doesn't try to read policy.csv from disk.
...
// setupQueryService creates a full Service with all dependencies using a temp SQLite DB.
...
func seedQueryDatasource(t *testing.T, svc *datasource.Service, ctx context.Context) int64 {
	t.Helper()
	ds := &model.DataSource{
...
func seedQueryDatasourceWithStatus(t *testing.T, svc *datasource.Service, ctx context.Context, status string) int64 {
	t.Helper()
	ds := &model.DataSource{
		Name: "query-test-ds", Type: "mysql", Host: "10.0.0.1", Port: 3306,
		Username: "root", PasswordEncrypted: "secret", Database: "testdb",
...
func TestTruncateSQL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := auditlog.Summarize(tt.input)
			if got != tt.want {
				t.Errorf("auditlog.Summarize() length = %d, want %d", len(got), len(tt.want))
			}
		})
	}
}
...
func TestApplyDesensitization_NoRules(t *testing.T) {
	...
	desensitized, maskedFields, maskErr := applyRelease(t, qs, ctx, testActorID, "developer", 1, result, []string{"users"})
	if maskErr != nil {
...
func TestApplyDesensitization_WithRules(t *testing.T) {
	...
	desensitized, maskedFields, maskErr := applyRelease(t, qs, ctx, testActorID, "developer", dsID, result, []string{"users"})
	if maskErr != nil {
		t.Fatalf("release decision: %v", maskErr)
	}
	if !desensitized {
...
func TestApplyDesensitization_BypassPermission(t *testing.T) {
	...
	desensitized, maskedFields, maskErr := applyRelease(t, qs, ctx, testActorID, "admin", dsID, result, []string{"users"})
	if maskErr != nil {
		t.Fatalf("release decision: %v", maskErr)
	}
	if desensitized {
...
func TestApplyDesensitization_WildcardTableRule(t *testing.T) {
	...
	desensitized, maskedFields, maskErr := applyRelease(t, qs, ctx, testActorID, "developer", dsID, result, []string{"orders"})
	if maskErr != nil {
		t.Fatalf("release decision: %v", maskErr)
	}
	if !desensitized {
...
func TestApplyDesensitization_EmptyTables(t *testing.T) {
	...
	desensitized, maskedFields, maskErr := applyRelease(t, qs, ctx, testActorID, "developer", 1, result, []string{})
	if maskErr != nil {
		t.Fatalf("release decision: %v", maskErr)
	}
	if desensitized {
...
func applyRelease(t *testing.T, qs *Service, ctx context.Context, userID int64, role string, dsID int64, result *QueryResult, tables []string) (bool, []string, error) {
	t.Helper()
	d, err := releaseRows(ctx, qs.client, qs.permSvc, releaseActor{UserID: userID, Role: role},
		dsID, "testdb", tables, result.Shape, result.Rows)
	if err != nil {
	...
	return d.Verdict == releaseMasked, d.MaskedFields, nil
}
