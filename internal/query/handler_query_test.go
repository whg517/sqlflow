...
// setupQueryTest creates a fresh DB, services, and Handler for testing.
...
func seedTestDatasource(t *testing.T, dsSvc *datasource.Service, name string) *model.DataSource {
	t.Helper()
	ctx := testutil.ContextWithTimeout(t)
	ds := &model.DataSource{
		Name:     name,
...
func TestQueryHandler_ExecuteQuery_DisabledDatasource(t *testing.T) {
	...
	// Disabled datasource → datasource.ErrDatasourceDisabled → handler maps to internal error
	if rec.Code != http.StatusBadRequest {
		t.Errorf("status = %d, want %d; body = %s", rec.Code, http.StatusBadRequest, rec.Body.String())
	}
}
...
func TestQueryHandler_ClearHistory_Success(t *testing.T) {
	...
	result2 := testutil.DecodeJSON(t, rec2)
...
func TestQueryHandler_ExportQuery_HighRiskSQL(t *testing.T) {
	...
	// ExecuteQuery 先检查 IsBlocked → 返回 ErrSQLBlocked → handler 返回 400。
