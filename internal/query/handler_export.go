...
type ExportHandler struct {
	...
	exportAsyncSvc *AsyncExportService
}

// NewExportHandler creates a new ExportHandler.
func NewExportHandler(exportSvc *ExportService, exportAsyncSvc *AsyncExportService) *ExportHandler {
...
func (h *ExportHandler) ExportAuditLogs(c echo.Context) error {
	...
	exportFormat := ParseExportFormat(req.Format)
	var columns []string
	if req.Columns != "" {
		if err := json.Unmarshal([]byte(req.Columns), &columns); err != nil {
			return resp.BadRequest(c, "columns 参数格式错误，应为 JSON 数组")
		}
		if _, err := ValidateExportColumns(columns, ExportTypeAudit); err != nil {
			return resp.BadRequest(c, err.Error())
		}
	}
	columnsMap, _ := ValidateExportColumns(columns, ExportTypeAudit)
...
func (h *ExportHandler) ExportTickets(c echo.Context) error {
	...
	if req.Columns != "" {
		...
		if _, err := ValidateExportColumns(columns, ExportTypeTicket); err != nil {
			return resp.BadRequest(c, err.Error())
		}
	}
	columnsMap, _ := ValidateExportColumns(columns, ExportTypeTicket)
