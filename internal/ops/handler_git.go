...
func NewGitHandler(gitSvc *GitService) *GitHandler {
...
func (h *GitHandler) CreateGitLink(c echo.Context) error {
	...
	input := CreateGitLinkInput{
		...
		LinkType:    ToGitLinkType(req.LinkType),
	...
	if err != nil {
		switch err {
		case ErrGitLinkEntityTypeInvalid,
			ErrGitLinkTypeInvalid,
			ErrGitLinkCommitHashRequired:
