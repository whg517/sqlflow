...
type AIReviewHandler struct {
	aiReviewSvc *AIReviewService
	dsSvc       *datasource.Service
}

// NewAIReviewHandler creates a new AIReviewHandler.
func NewAIReviewHandler(aiReviewSvc *AIReviewService, dsSvc *datasource.Service) *AIReviewHandler {
...
func (h *AIReviewHandler) ReviewStream(c echo.Context) error {
	...
	// Let the datasource's own driver interpret the query.
	parseResult, err := driver.ParseFor(dbType, req.SQL)
	if err != nil {
