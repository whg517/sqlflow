...
// setupTicketHandlerTest creates a fresh DB, services, and Handler for testing.
func setupTicketHandlerTest(t *testing.T) (*echo.Echo, *Handler, *db.DB) {
	t.Helper()
	...
	ticketSvc := New(Deps{DB: wrapped, Audit: auditSvc})
	handler := NewHandler(ticketSvc)
...
// TestTicketHandler_CreateTicket_IgnoresClientRiskLevel is the API-level
// regression for REV-P1-006. Risk selects the approval policy — including
// auto-approval — so a submitter who could declare their SQL "low" could also
// choose their own approval path. The server scores the statement itself and
// ignores anything the client sends.
func TestTicketHandler_CreateTicket_IgnoresClientRiskLevel(t *testing.T) {
	...
	ctx := testutil.ContextWithTimeout(t)
	var risk, aiReview string
	err := database.QueryRowContext(ctx,
		`SELECT risk_level, ai_review_result FROM tickets ORDER BY id DESC LIMIT 1`,
	).Scan(&risk, &aiReview)
	if err != nil {
	...
	if risk == "low" {
		t.Error(`stored risk_level = "low" — the client's declared value was honored`)
	}
	if risk == "" {
		t.Error("stored risk_level is empty, want a server-derived level")
	}
	if aiReview != "" {
		t.Errorf("stored ai_review_result = %q, want empty — AI verdicts must not be client-supplied", aiReview)
	}
}
...
func TestTicketHandler_FullWorkflow(t *testing.T) {
	...
	createResult := testutil.DecodeJSON(t, rec)
