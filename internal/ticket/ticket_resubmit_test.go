...
func TestResubmitTicket(t *testing.T) {
	...
	t.Run("success - resubmit rejected ticket", func(t *testing.T) {
		...
		// The new revision must be re-analyzed, not blanked: risk drives
		// approval-policy matching, and approvers read sql_type/affected_tables
		// to judge impact. Leaving them empty or stale describes the wrong SQL.
		wantRisk := NewRiskEvaluator().Evaluate(
			NewSQLAnalyzer().Analyze("DELETE FROM users WHERE status = 0")).Level
		if result.RiskLevel != wantRisk {
			t.Errorf("RiskLevel = %q, want re-derived %q", result.RiskLevel, wantRisk)
		}
		if result.SQLType == "" {
			t.Error("SQLType should be re-derived from the new SQL, got empty")
		}
		if result.SQLHash != "" {
			t.Errorf("SQLHash = %q, want empty — the new revision is not approved", result.SQLHash)
		}
