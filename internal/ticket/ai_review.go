...
func (s *AIReviewService) Review(ctx context.Context, req *AIReviewRequest) (*AIReviewResult, error) {
	...
	if s.config.Enabled {
		...
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, ErrAIReviewTimeout) {
			log.Printf("AI review timeout, degrading to static rules for SQL: %s", auditlog.Summarize(req.SQL))
...
func determineStaticRisk(pr *driver.ParseResult, isSensitive bool, role string) string {
	...
	case driver.OpDDL:
		return AIRiskHigh
	case driver.OpUpdate, driver.OpDelete, driver.OpDML:
		if pr.RiskLevel == driver.RiskHigh {
			return AIRiskHigh
		}
		if isSensitive {
			return AIRiskHigh
...
func generateStaticSuggestions(pr *driver.ParseResult, isSensitive bool) []string {
	suggestions := make([]string, 0)

	if pr.Operation == driver.OpSelect {
	...
	if pr.Operation == driver.OpUpdate || pr.Operation == driver.OpDelete {
		suggestions = append(suggestions, "变更操作需通过工单审批流程")
	}

	if pr.Operation == driver.OpDDL {
...
func generateStaticSummary(pr *driver.ParseResult, isSensitive bool, riskLevel string) string {
	opName := pr.Operation
	if opName == "" {
		opName = "unknown"
	}
	...
	if len(pr.Targets) > 0 {
		parts = append(parts, fmt.Sprintf("涉及表: %s", strings.Join(pr.Targets, ", ")))
	}
...
func RiskToDecision(risk string, op string) ReviewDecision {
	switch risk {
	case AIRiskHigh:
		if op == driver.OpSelect {
...
func (s *AIReviewService) degradeResult(staticResult *AIReviewResult, req *AIReviewRequest) *AIReviewResult {
	...
	if result.RiskLevel == AIRiskLow && req.ParseResult != nil &&
		req.ParseResult.Operation != driver.OpSelect {
		result.RiskLevel = AIRiskMedium
	}
