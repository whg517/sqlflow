...
// setupDatasourceTest creates a fresh Echo, DB, DatasourceService, and Handler for testing.
...
func createTestDatasource(t *testing.T, dsSvc *Service, name, dsType, host string, port int) *model.DataSource {
	t.Helper()
...
// TestDatasourceHandler_GetCapabilities is the contract the query UI reads to
// decide which editor and actions to present. It must distinguish Elasticsearch
// from SQL sources without the caller knowing type names.
func TestDatasourceHandler_GetCapabilities(t *testing.T) {
	e, dsSvc, h := setupDatasourceTest(t)

	tests := []struct {
		dsType         string
		wantQueryForm  string
		wantTicketExec bool
	}{
		{"mysql", "sql", true},
		{"postgresql", "sql", true},
		{"elasticsearch", "dsl", false},
		{"mongodb", "document", true},
	}

	for _, tt := range tests {
		t.Run(tt.dsType, func(t *testing.T) {
			ds := createTestDatasource(t, dsSvc, "cap-"+tt.dsType, tt.dsType, "localhost", 3306)

			req := httptest.NewRequest(http.MethodGet, "/api/datasources/"+fmt.Sprint(ds.ID)+"/capabilities", nil)
			rec := httptest.NewRecorder()
			c := e.NewContext(req, rec)
			c.SetParamNames("id")
			c.SetParamValues(fmt.Sprint(ds.ID))

			if err := h.GetDatasourceCapabilities(c); err != nil {
				t.Fatalf("handler error: %v", err)
			}
			if rec.Code != http.StatusOK {
				t.Fatalf("status = %d, want 200; body = %s", rec.Code, rec.Body.String())
...
func TestDatasourceHandler_GetCapabilities_NotFound(t *testing.T) {
	e, _, h := setupDatasourceTest(t)

	req := httptest.NewRequest(http.MethodGet, "/api/datasources/99999/capabilities", nil)
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)
	c.SetParamNames("id")
	c.SetParamValues("99999")
