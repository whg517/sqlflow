...
// PlatformPermissionLister returns the platform-level permission keys granted
// to a role.
//
// Declared at the point of use: the profile endpoint is the only place iam asks
// anything of authorization, and naming just that keeps identity independent of
// the security domain.
type PlatformPermissionLister interface {
	GetPlatformPermissions(role string) ([]string, error)
}

// UserHandler handles user/auth related requests.
type UserHandler struct {
	authSvc *Service
	permSvc PlatformPermissionLister
}

// NewUserHandler creates a new UserHandler.
//
// permission is variadic because the profile endpoint predates platform
// permissions; without it the permission list comes back empty.
func NewUserHandler(authSvc *Service, permission ...PlatformPermissionLister) *UserHandler {
...
func (h *UserHandler) Refresh(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, ErrRefreshTokenInvalid) || errors.Is(err, ErrRefreshTokenRevoked) {
