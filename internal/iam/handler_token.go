...
func NewTokenHandler(tokenSvc *TokenService) *TokenHandler {
...
func (h *TokenHandler) CreateToken(c echo.Context) error {
	userID, _, _, err := httpx.RequireAuth(c)
	if err != nil {
		return err
	}
	...
	if err != nil {
		if errors.Is(err, ErrTokenNameExists) {
			return c.JSON(http.StatusConflict, resp.ErrorResponse{
				Code:    409,
				Message: "同名 Token 已存在",
			})
		}
		if errors.Is(err, ErrTokenInvalidScope) {
