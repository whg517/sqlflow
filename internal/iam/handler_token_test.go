...
func setupTokenTest(t *testing.T) (*echo.Echo, *TokenService, *TokenHandler, int64) {
	t.Helper()
	d := testutil.NewDB(t)

	authSvc := NewService(d, "test-jwt-secret-32byteslong!", 0)
	tokenSvc := NewTokenService(d)
...
func newTokenContext(e *echo.Echo, method, path, body string, userID int64) (echo.Context, *httptest.ResponseRecorder) {
	...
	testutil.SetContextUser(c, userID, "tokenowner", "developer")
	return c, rec
}
