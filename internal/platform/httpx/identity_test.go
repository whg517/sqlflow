...
// TestSafeContextExtraction_NilContext verifies that UserID/Username/Role
...
func TestSafeContextExtraction_CorrectValues(t *testing.T) {
	...
	if id := UserID(c); id != 42 {
		t.Errorf("expected 42, got %d", id)
	}
	if name := Username(c); name != "testuser" {
		t.Errorf("expected testuser, got %q", name)
	}
	if role := Role(c); role != "admin" {
