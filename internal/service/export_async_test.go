...
CREATE TABLE IF NOT EXISTS export_tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, username TEXT NOT NULL DEFAULT '', export_type TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'pending', filename TEXT NOT NULL DEFAULT '', file_path TEXT NOT NULL DEFAULT '', total_rows INTEGER NOT NULL DEFAULT 0, file_bytes INTEGER NOT NULL DEFAULT 0, filters_json TEXT NOT NULL DEFAULT '{}', error_msg TEXT NOT NULL DEFAULT '', created_at DATETIME NOT NULL DEFAULT (datetime('now')), completed_at DATETIME);
	`)
	if err != nil {
...
func TestExportAsyncService_CreateAndRetrieve(t *testing.T) {
	...
	task, err := asyncSvc.CreateAsyncExport(context.Background(), 1, "admin", "admin", "audit", string(filtersJSON))
	if err != nil {
...
func TestExportAsyncService_ListTasks(t *testing.T) {
	...
	_, err = asyncSvc.CreateAsyncExport(context.Background(), 1, "admin", "admin", "audit", string(filtersJSON))
	if err != nil {
		t.Fatalf("CreateAsyncExport: %v", err)
	}
...
func TestExportAsyncService_PermissionDenied(t *testing.T) {
	...
	filtersJSON, _ := json.Marshal(AuditExportFilters{})
	_, err := asyncSvc.CreateAsyncExport(context.Background(), 2, "dev", "developer", "audit", string(filtersJSON))
...
func TestGenerateExportFilename(t *testing.T) {
	fname := generateExportFilename("audit")
	if fname == "" {
		t.Error("expected non-empty filename")
	}
...
func TestExportAsyncService_CleanupExpiredFiles(t *testing.T) {
	...
	filtersJSON, _ := json.Marshal(AuditExportFilters{})
	task, _ := asyncSvc.CreateAsyncExport(context.Background(), 1, "admin", "admin", "audit", string(filtersJSON))
