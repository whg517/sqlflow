...
func setupAuditReportTest(t *testing.T) (*echo.Echo, *service.AuditReportService, *AuditReportHandler, int64) {
	t.Helper()
	d := testutil.NewDB(t)

	reportSvc := service.NewAuditReportService(d)
	h := NewAuditReportHandler(reportSvc)
	e := echo.New()
...
func newAuditReportCtx(e *echo.Echo, path string, userID int64) (echo.Context, *httptest.ResponseRecorder) {
	...
	setContextUser(c, userID, "alice", "admin")
	return c, rec
}
...
func TestAuditReport_GetUsageStats_EmptyDB(t *testing.T) {
	d := testutil.NewDB(t)
	h := NewAuditReportHandler(service.NewAuditReportService(d))
	e := echo.New()
