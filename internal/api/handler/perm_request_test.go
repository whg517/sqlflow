...
func setupPermReqTest(t *testing.T) (*echo.Echo, *service.PermissionRequestService, *PermReqHandler) {
	t.Helper()
	d := testutil.NewDB(t)
	permSvc, err := service.NewPermissionService(d)
	if err != nil {
		t.Fatalf("NewPermissionService: %v", err)
	}
	svc := service.NewPermissionRequestService(d, permSvc, nil)
	h := NewPermReqHandler(svc)
...
func TestPermReqHandler_CreateRequest_ValidationErrors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			setContextUser(c, 1, "dev1", "developer")
