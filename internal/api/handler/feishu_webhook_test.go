...
func setupFeishuWebhookTest(t *testing.T) (*echo.Echo, *service.FeishuWebhookService, *FeishuWebhookHandler) {
	t.Helper()
	d := testutil.NewDB(t)
	svc := service.NewFeishuWebhookService(d.DB, "test-encryption-key-32byte-len!!")
	h := NewFeishuWebhookHandler(svc)
