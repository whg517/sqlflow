...
func TestAuditService_WriteSync(t *testing.T) {
	...
	svc := NewService(testutil.WrapSQL(t, db), 0, 0)
...
func TestAuditService_List_NoFilters(t *testing.T) {
	...
	svc := NewService(testutil.WrapSQL(t, db), 100, 50*time.Millisecond)
	...
	svc2 := NewService(testutil.WrapSQL(t, db), 100, 50*time.Millisecond)
	defer svc2.Close()
...
func TestEscapeLike(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := sqlutil.EscapeLike(tt.input)
			if got != tt.want {
				t.Errorf("sqlutil.EscapeLike(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}
...
func TestAuditService_CloseIsNoop(t *testing.T) {
	...
	svc := NewService(testutil.WrapSQL(t, db), 0, 0)
	svc.Write(context.Background(), auditlog.Record{UserID: 1, Action: "query_execute"})
	svc.Close()
...
func TestAuditService_Write_NilReceiver(t *testing.T) {
	...
	var svc *Service
	svc.Write(context.Background(), auditlog.Record{UserID: 1, Action: "test"})
}
...
func TestAuditService_List_FilterByTimeRange(t *testing.T) {
	...
	svc.Write(context.Background(), auditlog.Record{UserID: 1, Action: "query_execute", SQLContent: "SELECT 1"})
