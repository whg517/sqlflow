...
// setupAuditTest creates a fresh Echo, DB, AuditService, and Handler for testing.
...
func seedAuditLogs(t *testing.T, auditSvc *Service, n int) {
	t.Helper()
	for i := 0; i < n; i++ {
		auditSvc.Write(testutil.ContextWithTimeout(t), auditlog.Record{
			UserID:          1,
...
func TestAuditHandler_ListAuditLogs_FilterCombinations(t *testing.T) {
	...
	for _, r := range records {
		auditSvc.Write(testutil.ContextWithTimeout(t), r)
	}
