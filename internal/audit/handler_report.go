...
// ReportHandler handles audit report requests.
...
func (h *ReportHandler) GetUsageStats(c echo.Context) error {
	days, _ := strconv.Atoi(c.QueryParam("days"))

	stats, err := h.reportSvc.GetUsageStats(c.Request().Context(), ReportParams{Days: days})
	if err != nil {
...
func (h *ReportHandler) GetErrorStats(c echo.Context) error {
	days, _ := strconv.Atoi(c.QueryParam("days"))

	stats, err := h.reportSvc.GetErrorStats(c.Request().Context(), ReportParams{Days: days})
	if err != nil {
...
func (h *ReportHandler) GetPerformanceReport(c echo.Context) error {
	days, _ := strconv.Atoi(c.QueryParam("days"))

	stats, err := h.reportSvc.GetPerformanceReport(c.Request().Context(), ReportParams{Days: days})
	if err != nil {
...
func (h *ReportHandler) GetTicketReport(c echo.Context) error {
	days, _ := strconv.Atoi(c.QueryParam("days"))

	stats, err := h.reportSvc.GetTicketReport(c.Request().Context(), ReportParams{Days: days})
	if err != nil {
...
func (h *ReportHandler) GetUserAnalytics(c echo.Context) error {
	timeRange := c.QueryParam("time_range")
	if timeRange == "" {
		timeRange = "7d"
	}

	userID, err := ParseAnalyticsUserID(c.QueryParam("user_id"))
	if err != nil {
