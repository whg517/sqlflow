...
func TestPermReqService_CreateRequest(t *testing.T) {
	...
	// Create a Service (needed for dependency)
	permSvc, err := NewService(testutil.WrapSQL(t, db))
	if err != nil {
		t.Fatalf("create Service: %v", err)
	}
	svc := NewRequestService(testutil.WrapSQL(t, db), permSvc, auditlog.Discard)
