...
// RequestHandler handles permission request endpoints.
...
func (h *RequestHandler) CreateRequest(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, ErrInvalidAction) || errors.Is(err, ErrInvalidDuration) {
...
func (h *RequestHandler) ListRequests(c echo.Context) error {
...
func (h *RequestHandler) MyRequests(c echo.Context) error {
	userID := httpx.UserID(c)
...
func (h *RequestHandler) MyActiveRequests(c echo.Context) error {
	userID := httpx.UserID(c)
...
func (h *RequestHandler) GetRequest(c echo.Context) error {
	...
	if err != nil {
		if errors.Is(err, ErrPermReqNotFound) {
			return resp.NotFound(c, "权限申请不存在")
		}
		return resp.InternalError(c, "查询失败")
	}
	role := httpx.Role(c)
	if pr.ApplicantID != httpx.UserID(c) && role != "admin" && role != "dba" {
...
func (h *RequestHandler) ApproveRequest(c echo.Context) error {
	approverID := httpx.UserID(c)
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的 ID")
	}
...
func (h *RequestHandler) RejectRequest(c echo.Context) error {
	approverID := httpx.UserID(c)
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的 ID")
...
func (h *RequestHandler) RevokeRequest(c echo.Context) error {
	revokerID := httpx.UserID(c)
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的 ID")
	}
...
func (h *RequestHandler) ExpireOverdue(c echo.Context) error {
