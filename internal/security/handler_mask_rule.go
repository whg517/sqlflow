...
// MaskHandler handles mask rule related requests.
type MaskHandler struct {
	maskRuleSvc *MaskService
}

// NewMaskHandler creates a new MaskHandler.
func NewMaskHandler(maskRuleSvc *MaskService) *MaskHandler {
	return &MaskHandler{maskRuleSvc: maskRuleSvc}
}
...
func (h *MaskHandler) CreateMaskRule(c echo.Context) error {
	...
	if err != nil {
		switch err {
		case ErrMaskRuleFieldRequired:
			return resp.BadRequest(c, err.Error())
		case ErrMaskRuleTableRequired:
			return resp.BadRequest(c, err.Error())
		case ErrMaskRuleTypeInvalid:
			return resp.BadRequest(c, err.Error())
		case ErrMaskRuleCustomRegexRequired:
			return resp.BadRequest(c, err.Error())
		case ErrMaskRuleDuplicate:
			return resp.BadRequest(c, err.Error())
		default:
...
func (h *MaskHandler) GetMaskRule(c echo.Context) error {
...
func (h *MaskHandler) ListMaskRules(c echo.Context) error {
...
func (h *MaskHandler) UpdateMaskRule(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的规则ID")
	}
...
func (h *MaskHandler) DeleteMaskRule(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的规则ID")
	}
...
func (h *MaskHandler) CreateSensitiveTable(c echo.Context) error {
	...
	if err != nil {
		switch err {
		case ErrSensitiveTableRequired:
			return resp.BadRequest(c, err.Error())
		case ErrSensitiveTableDuplicate:
			return resp.BadRequest(c, err.Error())
		case ErrInvalidSensitivityLevel:
			return resp.BadRequest(c, err.Error())
		default:
...
func (h *MaskHandler) ListSensitiveTables(c echo.Context) error {
	page, _ := strconv.Atoi(c.QueryParam("page"))
	pageSize, _ := strconv.Atoi(c.QueryParam("page_size"))
	if page <= 0 {
		page = 1
...
func (h *MaskHandler) DeleteSensitiveTable(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
	...
	if err := h.maskRuleSvc.DeleteSensitiveTable(c.Request().Context(), userID, id); err != nil {
		switch err {
		case ErrSensitiveTableNotFound:
			return resp.NotFound(c, err.Error())
		default:
