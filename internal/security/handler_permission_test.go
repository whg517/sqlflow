...
// setupPermissionTest creates a fresh Echo, DB, PermissionService, and Handler for testing.
...
func setupPermissionTest(t *testing.T) (*echo.Echo, *Service, *Handler) {
	...
	permSvc, err := NewService(database)
	if err != nil {
		t.Fatalf("create permission service: %v", err)
	}

	handler := NewHandler(permSvc)
...
func addTestPolicy(t *testing.T, permSvc *Service, sub, dom, obj, act string) {
	t.Helper()
