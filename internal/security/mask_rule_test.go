...
func newTestMaskRuleService(t *testing.T) (*MaskService, *sql.DB) {
	t.Helper()
	database := testutil.NewDB(t).DB
	svc := NewMaskService(testutil.WrapSQL(t, database), nil, auditlog.Discard)
	return svc, database
}
