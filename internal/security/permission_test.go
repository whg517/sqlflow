...
// NewService.seedIfEmpty sees a non-empty table and skips the file read.
...
// newTestPermissionService creates a Service backed by a temp SQLite DB
// with seed policies pre-loaded.
func newTestPermissionService(t *testing.T) (*Service, *sql.DB) {
	t.Helper()
	...
	svc, err := NewService(testutil.WrapSQL(t, testDB))
	if err != nil {
...
func TestNewPermissionService(t *testing.T) {
	...
	t.Run("does not re-seed on second creation", func(t *testing.T) {
		...
		_, err := NewService(testutil.WrapSQL(t, testDB))
		if err != nil {
...
func TestPermissionService_SeedIfEmpty_SeedsFromCSV(t *testing.T) {
	...
	// Table is empty, so NewService should trigger seedIfEmpty
	// which reads from the embedded FS or policy.csv file.
	svc, err := NewService(testutil.WrapSQL(t, testDB))
	if err != nil {
		t.Fatalf("NewService on empty table: %v", err)
	}
	_ = svc
