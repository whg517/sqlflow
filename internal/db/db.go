...
CREATE TABLE IF NOT EXISTS export_tasks (
	...
	`)
	if err != nil {
		return fmt.Errorf("migrate export_tasks: %w", err)
	}

	_, err = db.Exec(`CREATE INDEX IF NOT EXISTS idx_export_tasks_user ON export_tasks(user_id)`)
	if err != nil {
		return fmt.Errorf("migrate export_tasks user index: %w", err)
	}

	_, err = db.Exec(`CREATE INDEX IF NOT EXISTS idx_export_tasks_status ON export_tasks(status)`)
	if err != nil {
