...
func setupNotifPrefTest(t *testing.T) (*echo.Echo, *PreferenceService, *PreferenceHandler) {
	t.Helper()
	d := testutil.NewDB(t)
	svc := NewPreferenceService(d)
...
func TestNotificationPreferenceHandler_UpdatePreferences_Success(t *testing.T) {
	...
	testutil.SetContextUser(c2, 1, "alice", "developer")
