...
// FeishuHandler handles Feishu webhook CRUD API.
type FeishuHandler struct {
	svc *FeishuService
}

// NewFeishuHandler creates a new handler.
...
// @Produce json
// @Security BearerAuth
// @Param body body FeishuCreateRequest true "Webhook 配置"
...
func (h *FeishuHandler) Create(c echo.Context) error {
	var req FeishuCreateRequest
...
func (h *FeishuHandler) List(c echo.Context) error {
...
func (h *FeishuHandler) Get(c echo.Context) error {
...
// @Security BearerAuth
// @Param id path int true "Webhook ID"
// @Param body body FeishuUpdateRequest true "更新字段"
// @Success 200 {object} resp.SuccessResponse "更新成功"
// @Failure 400 {object} resp.ErrorResponse "请求错误"
// @Router /settings/feishu/webhooks/{id} [put]
func (h *FeishuHandler) Update(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的 ID")
	}

	var req FeishuUpdateRequest
	if err := c.Bind(&req); err != nil {
		return resp.BadRequest(c, "请求格式错误")
	}
...
func (h *FeishuHandler) Delete(c echo.Context) error {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resp.BadRequest(c, "无效的 ID")
	}
...
func (h *FeishuHandler) ListDeadLetters(c echo.Context) error {
	var webhookID int64
